...
function hasValidEstimate(task) {
  return task.optimistic !== null &&
    task.optimistic !== undefined &&
    task.mostLikely !== null &&
    task.mostLikely !== undefined &&
    task.pessimistic !== null &&
    task.pessimistic !== undefined;
}

export function showSimulationModal(task) {
  if (!hasValidEstimate(task)) {
    openModal(`
      <div class="relative">
      ...
      <p class="mt-6 text-sm leading-6 text-[var(--color-text)]/70">
        見積もりが未設定のため、シミュレーションできません。タスクを編集して最短・最頻・最長の日数を入力してください。
      </p>
    `);
